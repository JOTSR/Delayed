 ...
 */
export async function* interval<TArgs extends unknown[], TYield = unknown>(callback: Callback<TArgs, TYield>, delay: ms, ...args: TArgs): AsyncGenerator<TYield, void, void> {
    while (true) {
        await sleep(delay)
        yield callback(...args)
    }
